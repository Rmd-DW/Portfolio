    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between py-4">
          ...
          <div className="hidden lg:flex items-center space-x-1 bg-black/40 backdrop-blur-md rounded-2xl p-2 border border-white/10 shadow-2xl">
            {navItems.map((item) => (
              <a
                key={item.id}
                href={item.href}
          ...
          <button
            ...
            className="lg:hidden w-10 h-10 flex items-center justify-center bg-black/40 backdrop-blur-md rounded-lg border border-white/10 text-white hover:bg-white/10 transition-all duration-300"
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden pb-4 animate-in slide-in-from-top duration-300">
            <div className="bg-black/40 backdrop-blur-md rounded-2xl p-2 border border-white/10 shadow-2xl">
              {navItems.map((item) => (
                <a
                  key={item.id}
                  href={item.href}
                  ...
                  className={`
                    block px-4 py-3 rounded-xl text-sm font-medium transition-all duration-300 cursor-pointer mb-1
                    ${
                      activeSection === item.id
                        ? "bg-primary text-black shadow-lg shadow-primary/25"
                        : "text-white/80 hover:text-white hover:bg-white/10"
